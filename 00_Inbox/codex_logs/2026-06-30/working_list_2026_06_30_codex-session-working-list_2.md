...
- Codex CLI 0.142.4에서는 세션 종료 전용 hook을 확인하지 못했고, `Stop`은 turn 종료 hook으로만 확인했다.
- 기존 `hooks.json`과 plan policy hook은 유지하고, 대체 워크플로우로 helper script와 AGENTS 규칙만 추가했다.
...
- 다음 Codex 세션 종료 전에 `~/.codex/bin/codex-working-list`를 실행해 요약을 저장한다.

## 8. Next Suggested Step

- 다음 세션에서 helper를 직접 실행해 Obsidian 유입 흐름을 확인한다.
- Codex가 실제 session-end hook을 노출하면 그때 `session_working_list.py`를 hook으로 연결할지 재검토한다.
